...
const GuessPanelButton = ({number = 0, onPress = () => {}}) => {
  return (
    <TouchableOpacity testID={`guess-${number}`} onPress={onPress}>
      <Text>{number}</Text>
    </TouchableOpacity>
  );
};
...
const GuessRemoveButton = ({onPress = () => {}}) => {
  return (
    <TouchableOpacity testID="guess-backspace" onPress={onPress}>
...
const GuessSubmitButton = ({onPress = () => {}}) => {
  return (
    <TouchableOpacity testID="guess-submit" onPress={onPress}>
      <Text>Submit</Text>
    </TouchableOpacity>
  );
};
...
const GuessPanel = ({
  ...
  const createGuessPanelButton = i => (
    <GuessPanelButton key={i} number={i} onPress={() => onInputGuess(i)} />
  );
  return (
    <>
